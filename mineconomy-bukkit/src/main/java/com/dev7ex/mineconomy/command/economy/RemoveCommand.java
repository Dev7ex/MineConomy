 ...
 * @since 14.06.2023
 */
@CommandProperties(name = "remove", permission = "mineconomy.command.economy.remove")
public class RemoveCommand extends BukkitCommand implements TabCompleter {
    ...
    public boolean execute(@NotNull final CommandSender commandSender, @NotNull final String[] arguments) {
        if (arguments.length != 3) {
            commandSender.sendMessage(super.getConfiguration().getString("commands.economy.remove.usage")
        ...
        if (!NumberUtil.isNumber(arguments[2])) {
            commandSender.sendMessage(super.getConfiguration().getString("commands.economy.remove.wrong-argument")
                    .replaceAll("%prefix%", super.getPrefix()));
            return true;
        }
        ...
        user.removeBalance(value);
        commandSender.sendMessage(super.getConfiguration().getString("commands.economy.remove.successfully-removed")
                .replaceAll("%prefix%", super.getPrefix())
