...
public class AddCommand extends BukkitCommand implements TabCompleter {
    ...
    public boolean execute(@NotNull final CommandSender commandSender, @NotNull final String[] arguments ) {
        if (arguments.length != 3) {
            commandSender.sendMessage(super.getConfiguration().getString("commands.economy.add.usage")
        ...
        if (!NumberUtil.isNumber(arguments[2])) {
            commandSender.sendMessage(super.getConfiguration().getString("commands.economy.add.wrong-argument")
                    .replaceAll("%prefix%", super.getPrefix()));
            return true;
        }
        ...
        commandSender.sendMessage(super.getConfiguration().getString("commands.economy.add.successfully-add")
                .replaceAll("%prefix%", super.getPrefix())
